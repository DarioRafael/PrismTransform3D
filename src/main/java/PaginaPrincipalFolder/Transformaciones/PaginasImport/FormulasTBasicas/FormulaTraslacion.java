...
public class FormulaTraslacion extends JFrame {
    ...
    public FormulaTraslacion() {
        setTitle("FÓRMULAS DE LA TRASLACIÓN 3D BÁSICA");
        ...
        // Título principal
        JLabel titleLabel = new JLabel("FÓRMULAS DE LA TRASLACIÓN 3D BÁSICA");
        ...
        JLabel matrizGeneral = new JLabel("<html><div style='text-align: center; font-family: Courier New; font-size: 14px;'>"
                ...
                + "<tr><td>[</td><td>Tx</td><td>Ty</td><td>Tz</td><td>&nbsp;1&nbsp;</td><td>]</td></tr>"
                + "</table>"
                + "<div style='margin-top: 10px; font-size: 12px; color: red;'>[X' Y' Z' 1] = [X+Tx Y+Ty Z+Tz 1]</div></div></html>");
        ...
        JLabel[] operaciones = {
                createContentLabel("X(1) + Y(0) + Z(0) + 1(Tx) = X + Tx"),
                createContentLabel("X(0) + Y(1) + Z(0) + 1(Ty) = Y + Ty"),
                createContentLabel("X(0) + Y(0) + Z(1) + 1(Tz) = Z + Tz"),
